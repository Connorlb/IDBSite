...
then
  ...
  echo "To start the server use ./serverRestart.sh"
  echo "To kill the server use ./serverRestart.sh --kill"
  echo "And of course, to access this page use ./serverRestart --help"
else
  ...
  then
        ...
        sleep 50s
        echo "Done. Men only want one thing and it's disgusting"
  fi
fi
